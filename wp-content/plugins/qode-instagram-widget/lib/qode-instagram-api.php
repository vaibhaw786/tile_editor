...
class QodeInstagramApi {
    ...
    private function fetchData($limit = '', $tag = '', $transient = array('use_transient' => false)) {
        ...
        if($this->useTransients($transient) && get_transient($transient['transient_name'])) {
            ...
            if ($tag !== '') {
                ...
                $httpResponse = wp_remote_get($this->apiURL . '/tags/' . $tag . '/media/recent/?access_token=' . $this->accessToken . '&count=' . $limit, $args);
            } else {
                //request data from API
                $httpResponse = wp_remote_get($this->apiURL . '/users/' . $this->userID . '/media/recent/?access_token=' . $this->accessToken . '&count=' . $limit, $args);
            }
	 ...
	 *
	 */
	public function deleteFieldsFromDB() {

		$responseObj = new stdClass();

		$code_option_name				= delete_option( self::CODE_OPTION_NAME );
		$user_id_option_name			= delete_option( self::USER_ID_OPTION_NAME );
		$access_token_option_name		= delete_option( self::ACCESS_TOKEN_OPTION_NAME );
		$code_redirect_uri_option_name	= delete_option( self::CODE_REDIRECT_URI_OPTION_NAME );

		if($code_option_name && $user_id_option_name && $access_token_option_name && $code_redirect_uri_option_name) {
			$responseObj->status = true;
			$responseObj->message = esc_html__('Fields are deleted', 'qode-instagram-widget');
		} else {
			$responseObj->status = false;
			$responseObj->message = esc_html__('Fields are not deleted', 'qode-instagram-widget');
		}
